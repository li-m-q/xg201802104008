package src.cn.edu.sdjzu.xg.bysj.controller.basic.user;


import com.alibaba.fastjson.JSON;
import com.alibaba.fastjson.JSONObject;
import src.cn.edu.sdjzu.xg.bysj.domain.User;
import src.cn.edu.sdjzu.xg.bysj.service.DegreeService;
import src.cn.edu.sdjzu.xg.bysj.service.UserService;
...
public class UserController extends HttpServlet {
    ...
    //{"description":"","no":"05","remarks":""}
    //请使用以下JSON测试修改功能
    //{"description":"","id":1,"no":"05","remarks":""}

    /**
     * POST, http://localhost:8080/school.ctl, 增加
     * 增加对象：将来自前端请求的JSON对象，增加到数据库表中
     * @param request 请求对象
     * @param response 响应对象
    ...
    /**
     * DELETE, http://localhost:8080/school.ctl?id=1, 删除id=1的学位
    ...
    /**
     * PUT, http://localhost:8080/degree.ctl, 修改
     *
    ...
    /**
     * GET, http://localhost:8080/school.ctl?id=1, 查询id=1的学位
     * GET, http://localhost:8080/school.ctl, 查询所有的学位
     * 把一个或所有学位对象响应到前端
     * @param request
     * @param response
     * @throws ServletException
    ...
    protected void doGet(HttpServletRequest request, HttpServletResponse response)
        ...
        try {
            ...
            if (id_str == null) {
               responseUser1(username,response);
            } else {
                int id = Integer.parseInt(id_str);
    ...
    private void responseUser1(String username, HttpServletResponse response)
            throws ServletException, IOException, SQLException, ClassNotFoundException {
        //根据username查找
        User  user = UserService.getInstance().findByUsername(username);
        String degree_json = JSON.toJSONString(user);
