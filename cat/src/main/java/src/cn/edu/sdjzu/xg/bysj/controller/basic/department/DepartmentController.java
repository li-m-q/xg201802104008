package src.cn.edu.sdjzu.xg.bysj.controller.basic.department;

import com.alibaba.fastjson.JSON;
import com.alibaba.fastjson.JSONObject;
import com.alibaba.fastjson.serializer.SerializerFeature;
import src.cn.edu.sdjzu.xg.bysj.domain.Department;
import src.cn.edu.sdjzu.xg.bysj.service.DepartmentService;
...
//    DELETE, http://localhost:8080/department.ctl?id=1, 删除id=1的系
    ...
    protected void doGet(HttpServletRequest request, HttpServletResponse response)
        ...
        JSONObject message = new JSONObject();
        try {
            //如果id = null, 并且school_id =null表示响应所有学院对象
            if (id_str != null) {
                int id = Integer.parseInt(id_str);
                if(paraType!=null){
                    if (paraType.equals("school")){
                        responseSch_Department(id,response);
                    }
                    }
                else{
    ...
    private void responseSch_Department(int id, HttpServletResponse response)
            throws ServletException, IOException, SQLException {
        //根据id查找学院
